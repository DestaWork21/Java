...
<html>
	...
	<body>
		...
		<p>People who are atteding the event:${event.users.size} </p>
        <table>
            <tr>
                <th>Name:</th>
                <th>Location:</th>
        ...
        <h1>Message Wall</h1>
        <c:forEach items="${event.messages}" var="mess">
        <p>${mess.text}</p>
        </c:forEach>
        <form:form action="/events/${event.id}/messages/new" method="post" modelAttribute="message">
            <form:lebel path="text"> Add Message:
                <form:error path="input"></form:error>
